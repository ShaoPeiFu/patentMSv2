...
export const useSearchStore = defineStore("search", () => {
  ...
  const addSearchHistory = (
    ...
    const existingIndex = searchHistory.value.findIndex(
      (h) =>
        h.query === query &&
        JSON.stringify(h.conditions) === JSON.stringify(conditions)
    );
  ...
  const saveSearchConditions = (
    params: Omit<AdvancedSearchParams, "id" | "createdAt" | "useCount">
  ) => {
  ...
  const updateSavedSearch = (
    searchId: string,
    updates: Partial<AdvancedSearchParams>
  ) => {
    const search = savedSearches.value.find((s) => s.id === searchId);
    if (search) {
  ...
  // 更新分类选项
  const updateCategoryOptions = (categories: any[]) => {
    const categoryField = searchFields.value.find(
      (f) => f.key === "categoryId"
    );
    if (categoryField) {
