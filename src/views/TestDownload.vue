    ...
    <el-card>
      ...
      <div class="test-section">
        ...
        <div class="test-buttons">
          <el-button 
            type="primary" 
            @click="testDownloadApplication"
          >
            下载申请文件
          </el-button>
          <el-button 
            type="success" 
            @click="testDownloadPublication"
          >
            下载公开文件
          </el-button>
          <el-button 
            type="warning" 
            @click="testDownloadGrant"
          >
            下载授权文件
          </el-button>
          <el-button 
            type="info" 
            @click="testDownloadAmendment"
          >
            下载修改文件
          </el-button>
        </div>
      ...
      <div class="test-section">
        ...
        <el-table :data="testPatent.documents" stripe>
          ...
          <el-table-column label="操作" width="120">
            <template #default="{ row }">
              <el-button size="small" type="primary" @click="testDownloadDocument(row)">
...
import { downloadPatentDocument, downloadMultipleDocuments } from "@/utils/download";
...
const testPatent = ref<Patent>({
  id: 1,
  title: "一种智能专利管理系统",
  description: "本发明涉及一种智能专利管理系统，包括专利申请、审核、管理等功能。",
  ...
  inventors: ["李四", "王五"],
  abstract: "本发明提供了一种智能专利管理系统，通过人工智能技术实现专利的自动分类、审核和管理，提高了专利管理效率。",
  claims: [
...
const testDownloadApplication = async () => {
  const document = testPatent.value.documents.find(d => d.type === 'application');
  if (document) {
...
const testDownloadPublication = async () => {
  const document = testPatent.value.documents.find(d => d.type === 'publication');
  if (document) {
    await testDownloadDocument(document);
  }
};
...
const testDownloadAmendment = async () => {
  const document = testPatent.value.documents.find(d => d.type === 'amendment');
  if (document) {
    await testDownloadDocument(document);
  }
};
...
const testBatchDownload = async () => {
  try {
    ElMessage.info(`正在准备批量下载 ${testPatent.value.documents.length} 个文档...`);
    
    await downloadMultipleDocuments(testPatent.value.documents, testPatent.value, {
      showProgress: true
    });
    
    ElMessage.success(`批量下载完成，共 ${testPatent.value.documents.length} 个文档`);
  } catch (error) {
